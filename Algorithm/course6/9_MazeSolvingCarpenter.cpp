...
Point prePoint[MAX][MAX];//calculate distance by prePoint
...
void bfs(int y, int x) {
	...
	while (!qNotVisitedYet.empty()) {
		...
		//UP : Point(y-1, x)
		if ((0 <= y - 1 && y - 1 < Y) && !bDiscovered[y - 1][x]) {
			Point pNVY(y - 1, x);
			if (board[y - 1][x] == 0)
		...
		//RIGHT : Point(y, x+1)
		if ((0 <= x + 1 && x + 1 < X) && !bDiscovered[y][x + 1]) {
			Point pNVY(y, x + 1);
			if (board[y][x + 1] == 0)
				qNotVisitedYet.push(pNVY);
...
int main() {
	...
	//calculate distance from Start Co-ordinate(Y-1, 0) to Wall
	int cnt;
	for (int j = 0; j < Y; j++) {
		for (int i = 0; i < X; i++) {
	...
	//calculate distance from Wall to End Co-ordinate(0, X-1)
	int minDistance = 987654321;
	for (int j = 0; j < Y; j++) {
		for (int i = 0; i < X; i++) {
			if (board[j][i] != 0 && bDiscovered[j][i] && bDistance[j][i][0] == true) {// && bDiscovered[j][i] && bDistance[j][i][0] = true
				cnt = 0;
				cnt = countPathLen(0, X - 1, j, i);
				board[j][i] += cnt;
				if(board[j][i] < minDistance)
...
int main() {
	...
	//calculate distance from Start Co-ordinate(Y-1, 0) to Wall             //start co-ordinate(Y-1, 0) to Wall distance
	int cnt;
	for (int j = 0; j < Y; j++) {
		for (int i = 0; i < X; i++) {
			if (board[j][i] != 0 && bDiscovered[j][i]) {
	...
	//calculate distance from Wall to End Co-ordinate(0, X-1)                //end co-ordinate(0, X-1) to Wall distance
	for (int j = 0; j < Y; j++)
		for (int i = 0; i < X; i++) {
			bDiscovered[j][i] = false;
		}
